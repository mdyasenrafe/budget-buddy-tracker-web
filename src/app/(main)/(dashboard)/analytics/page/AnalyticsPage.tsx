...
export const AnalyticsPage = () => {
  ...
  const transactionFilters = useMemo(() => {
    ...
    if (selectedCategory?.id) {
      filters.push({ name: "category", value: selectedCategory.id });
      filters.push({ name: "type", value: selectedCategory.type });
    }

    return filters;
  }, [monthStart, monthEnd, selectedCategory]);

  const transactionHistoryExtra = selectedCategory ? (
    ...
    <Text variant="p5" className="text-xs text-slate-400 font-medium">
      {monthFormatted}
    </Text>
  );
  ...
  return (
    <div className="pb-24 pt-8 lg:pb-10">
      ...
      <div ref={transactionsSectionRef} className="mt-8 scroll-mt-6">
        <ChartCard
          title={
            selectedCategory
              ? `${selectedCategory.label} Transactions`
              : "Transaction History"
          }
          extra={transactionHistoryExtra}
          className="!mb-20 rounded-2xl border border-slate-200 bg-white shadow-sm transition-all"
        >
          {selectedCategory && (
          ...
          <TransactionsProvider
            key={`${monthStart}-${selectedCategory?.id || "all"}`}
